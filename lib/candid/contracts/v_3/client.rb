  ...
  module Contracts
    module V3
      class Client
        ...
        # Returns rendering providers linked to a contract with their credentialing spans, scoped to the contract's
        # contracting provider and payer. Providers with no matching spans are included with an empty list.
        #
        # @param request_options [Hash]
        # @param params [Hash]
        # @option request_options [String] :base_url
        ...
        # @option params [Integer, nil] :limit
        #
        # @return [Candid::Contracts::V3::Types::ContractProviderCredentialingPage]
        def get_contract_provider_credentialing_spans(request_options: {}, **params)
          params = Candid::Internal::Types::Utils.normalize_keys(params)
          query_param_names = %i[page_token limit]
          query_params = {}
          query_params["page_token"] = params[:page_token] if params.key?(:page_token)
          ...
          request = Candid::Internal::JSON::Request.new(
            base_url: request_options[:base_url] || @base_url || @environment&.dig(:candid_api),
            method: "GET",
            path: "/api/contracts/v3/#{params[:contract_id]}/providers/credentialing",
            query: query_params,
            request_options: request_options
          )
          begin
          ...
          code = response.code.to_i
          if code.between?(200, 299)
            Candid::Contracts::V3::Types::ContractProviderCredentialingPage.load(response.body)
          else
            error_class = Candid::Errors::ResponseError.subclass_for_code(code)
            raise error_class.new(response.body, code: code)
          end
