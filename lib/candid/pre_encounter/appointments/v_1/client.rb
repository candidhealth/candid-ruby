    ...
    module Appointments
      module V1
        class Client
          ...
          # @option params [String, nil] :filters
          #
          # @return [Candid::PreEncounter::Appointments::V1::Types::CountsResponse]
          def get_counts(request_options: {}, **params)
            params = Candid::Internal::Types::Utils.normalize_keys(params)
            ...
            code = response.code.to_i
            if code.between?(200, 299)
              Candid::PreEncounter::Appointments::V1::Types::CountsResponse.load(response.body)
            else
              error_class = Candid::Errors::ResponseError.subclass_for_code(code)
              raise error_class.new(response.body, code: code)
            end
